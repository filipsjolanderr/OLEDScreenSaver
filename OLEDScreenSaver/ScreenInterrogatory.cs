...
namespace OLEDScreenSaver
{
    public static class ScreenInterrogatory
    {
        ...
        private static string MonitorFriendlyName(LUID adapterId, uint targetId)
        {
            ...
            LogHelper.Log("Calling DisplayConfigGetDeviceInfo with " + deviceName);
        ...
        public static IEnumerable<string> GetAllMonitorsFriendlyNames()
        {
            ...
            if (error != ERROR_SUCCESS)
                throw new Win32Exception(error);
            LogHelper.Log("pathCount is " + pathCount + " and modeCount is " + modeCount);
            ...
            for (var i = 0; i < modeCount; i++)
                ...
                {
                    LogHelper.Log("MonitorFriendlyName with " + displayModes[i].adapterId + " " + displayModes[i].id);
